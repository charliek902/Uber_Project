...
public class Driver extends User {
    ...
    public Driver(Integer id, String fullName, GeoLocation location, String email, LocationService locationService, MessageService messageService, TripService tripService, Integer size, Trip driverTripType, ConnectionDB db) {
    ...
    public Response getRider(){
        Request request = new RequestBuilder(new Request())
                ...
                .setTimeOut(300)
                .setSize(this.size)
                .setRequestType(RequestType.REQUEST_RIDERS)
                .validate()
                .build();
    ...
    private GeoLocation randomizeLocation(Double longitude, Double latitude) {
        ...
        int steps = 100;
        Double newLongitude = movementSignLongitude == 1 ?  longitude + (Math.random() * steps) :  longitude + (Math.random() * (steps * -1));
